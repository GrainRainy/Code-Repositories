...
3. $rk < cnt(l(x))$: 这时 $x$ 右子树属于 $y$, 递归分裂左子树. 
...
尝试进行优化, 我们知道线段树的一个节点代表一个区间, 因此由一个点向区间连边可视为向线段树上的最多 $\log n$ 个节点连边. 由区间向点连边的情况与上述类似. 
...
最后从起始节点跑一遍最短路即可, 时间复杂度 $O(n \cdot \log^2 n)$

记得算好存图空间, 边数为 $O(4 \cdot n + n \cdot \log n)$.
