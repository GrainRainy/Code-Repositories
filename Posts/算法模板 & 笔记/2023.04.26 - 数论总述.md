...
2. $\backepsilon(n) = \begin{cases} 1 & x = 1 \\ 0 & x \ne 1 \end{cases}$
...
可以证明, $\text{id}(n)$, $\backepsilon(n)$ 与 $\sigma_k(n)$ 均为完全积性函数. 
...
$φ(n)$ 表示 $1$ ~ $n - 1$ 中与 $n$ 互质的数的个数. 

对于任意正整数 $n$, 有

$$φ(n) = n \times \prod\limits_{p \mid n} (1 - \frac{1}{p})$$
...
$$φ(n) = n \times (1 - \frac{1}{p_1}) \times (1 - \frac{1}{p_2}) \times (1 - \frac{1}{p_3}) ...$$
