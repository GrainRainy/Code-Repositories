...
void PushDown(int u) {
	if (tag(u)) {
		sum(lson) = (sum(lson) + tag(u) * (r(lson) - l(lson) + 1)) % mod;
		tag(lson) = (tag(lson) + tag(u)) % mod;
		sum(rson) = (sum(rson) + tag(u) * (r(rson) - l(rson) + 1)) % mod;
		tag(rson) = (tag(rson) + tag(u)) % mod;
		tag(u) = 0;
	}
}
...
void change(int u, int l, int r, int k) {
	...
	if (l <= l(u) and r >= r(u)) {
		sum(u) = (sum(u) + (LL)(r(u) - l(u) + 1) * k) % mod;
		tag(u) = (tag(u) + k) % mod;
		return;
	}
	PushDown(u);
...
inline LL QueryTree(int u) {
	return query(1, dfn[u], dfn[u] + siz[u] - 1) % mod;
}
...
int main() {
	...
	input(n), input(m), input(root), input(mod);
